...
export default (isComputeHeight) => {
  ...
  const applyScroll = () => {
    ...
    dom_lyric.value.scrollTop = scrollSpring.getCurrentPosition()
  }
  const onTick = (now) => {
  ...
  const getScrollTarget = () => {
    ...
    return dom_p.offsetTop - offset - getOffsetTop(dom_lyric.value.clientHeight, dom_p.clientHeight)
  }
  ...
  const handleMove = (x, y) => {
    if (isMsDown.value) {
      isStopScroll ||= true
      scrollSpring.setPosition(msDownScrollY + msDownY - y)
      applyScroll()
      ensureRaf()
