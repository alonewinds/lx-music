<template>
  <div class="action-buttons" @dblclick.stop>
    <button type="button" class="action-button" title="上一首" aria-label="上一首" @pointerdown.stop @pointerup.stop @pointercancel.stop @click.stop.prevent="emit('action', 'prev')">
    ...
    <button
      ...
      @pointerup.stop
      @pointercancel.stop
      @click.stop.prevent="emit('action', isPlaying ? 'pause' : 'play')"
    >
    ...
    <button type="button" class="action-button" title="下一首" aria-label="下一首" @pointerdown.stop @pointerup.stop @pointercancel.stop @click.stop.prevent="emit('action', 'next')">
      <svg viewBox="0 0 20 20" aria-hidden="true">
...
const emit = defineEmits<(event: 'action', action: 'prev' | 'next' | 'play' | 'pause') => void>()
</script>
