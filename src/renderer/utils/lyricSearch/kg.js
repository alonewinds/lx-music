/**
 * 酷狗音乐歌词搜索 API
 * 复用现有的 musicSdk/kg 模块
 */

import musicSearch from '../musicSdk/kg/musicSearch'
import lyric from '../musicSdk/kg/lyric'
...
const searchSongs = async (keyword, page = 1) => {
    ...
    const list = result.list.map(item => ({
        source,
        id: item.songmid?.toString() || item.hash,
        name: item.name,
        singer: item.singer,
        album: item.albumName,
        duration: item._interval ? item._interval * 1000 : parseInterval(item.interval),
        img: item.img,
        _extra: {
            hash: item.hash,
...
const getLyric = async (songInfo) => {
    const mInfo = {
        hash: songInfo._extra?.hash || songInfo.id,
