/**
 * 网易云音乐歌词搜索 API
 * 复用现有的 musicSdk/wy 模块
 */

import musicSearch from '../musicSdk/wy/musicSearch'
import lyric from '../musicSdk/wy/lyric'
...
const getLyric = async (songInfo) => {
    const songmid = songInfo._extra?.songmid || songInfo.id

    // lyric 模块默认导出是一个函数，直接调用
    const result = await lyric(songmid).promise
